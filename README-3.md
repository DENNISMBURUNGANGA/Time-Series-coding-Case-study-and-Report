...
The foundation model is omitted from this table deliberately — see Limitations.
...
**The residual diagnostics disagree with themselves, and the disagreement is
informative.** The residual ACF looks flat, but Ljung–Box rejects the no-autocorrelation
null at every lag tested: Q(24) = 40.92 (p = 0.017), Q(48) = 71.49 (p = 0.016),
Q(168) = 217.32 (p = 0.006).

These reconcile. With ~2,929 residuals the white-noise band on an individual
autocorrelation is ≈ ±0.036, while the Ljung–Box statistic implies a root-mean-square
autocorrelation of about 0.024 over the first 24 lags — inside the band, hence invisible
on the plot. Ljung–Box is a *joint* test and accumulates many small deviations into one
statistic. **The residual structure is real but small: too small to see, large enough to
detect.** The seasonal terms substantially reduced the daily dependence without
eliminating it, and the fitted model contains no weekly seasonal term at all despite the
weekly cycle being the dominant structure in this series — the most obvious untested
improvement.
...
- Ljung–Box rejects white noise at all lags tested, so the specification is incomplete;
  the missing weekly seasonal term is the obvious untested candidate
